...
def draw_frame(agent_pos, episode_num, step_num, obstacles, path=None):
    ...
    # Text Info
    cv2.putText(img, f"Episode: {episode_num} Step: {step_num}", (10, GRID_SIZE * CELL_SIZE + 35), 
...
def main():
    ...
    for episode in range(EPISODES):
        ...
        should_record = (episode % 50 == 0) or (episode > EPISODES - 2)
        
        steps = 0
        path = []
        
        # min score is now -200
        while not done and steps < 300: # Limit steps to prevent infinite loops early on
            path.append(state)
            if should_record:
    ...
    print(f"state: {state}")
    for _ in range(3): # Hold final result
        out.write(frame)
